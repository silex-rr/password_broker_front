...
const MainLeftMenuTreeNode = props => {
    ...
    switch (props.role) {
        ...
        case ROLE_ADMIN:
            ...
            canDrag = moveEntryGroupMode && true;
            canBeDropped = moveEntryGroupMode && true;
            break;
        case ROLE_MODERATOR:
    ...
    return (
        <li
            key={entryGroupIdCurrent}
            ref={drag}
            className="ml-2 border-l border-dotted border-slate-200 pl-2"
            ...
            <div ref={drop} className="tree-corner">
                ...
                <div className={'inline-block align-middle ' + (canDrag ? '' : 'w-2')}>
                    {canDrag ? <RiDraggable className={groupColor} /> : ''}
                </div>
