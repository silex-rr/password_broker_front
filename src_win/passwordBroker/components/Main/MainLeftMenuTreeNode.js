...
const MainLeftMenuTreeNode = props => {
    ...
    const {pid, lvl, last, parentLast} = props;
    const {
        // setEntryGroupId,
        entryGroupId,
        // setEntryGroupStatus,
    ...
    switch (props.role) {
        ...
        case ROLE_ADMIN:
            ...
            canDrag = moveEntryGroupMode;
            // eslint-disable-next-line no-unused-vars
            canBeDropped = moveEntryGroupMode;
            break;
        case ROLE_MODERATOR:
    ...
    const treePathUpper =
        lvl < 3 || parentLast ? (
            ...
            <React.Fragment>
                <View
                    style={{
                        ...tw`absolute bg-slate-100 h-full`,
                        ...{
                            top: -8,
                            left: -8,
                            opacity: 0.3,
        ...
        <View style={tw`relative`} key={entryGroupIdCurrent}>
            ...
            <View
                ...
                <View
                    ...
                    {
                        hasChildren ? (
                            <TouchableOpacity onPress={handleOpening} style={tw``}>
                                {
                                    isOpened ? (
                            ...
                            <MaterialCommunityIcons name="folder-outline" size={24} color={groupColor} />
                        ) //<HiOutlineFolder className={"inline-block text-2xl " + groupColor}/>
                    }

                    <View style={tw`${canDrag ? 'w-2' : ''}`}>
                        {canDrag ? (
                            <Text>draggable</Text> //<RiDraggable className={groupColor}/>
                        ) : (
                            ''
                        )}
                    </View>
                ...
                <View style={!isOpened ? tw`hidden` : {}}>{children}</View>
            </View>
        </View>
    );
};
