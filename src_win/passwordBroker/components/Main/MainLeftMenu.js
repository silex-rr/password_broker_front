...
const MainLeftMenu = () => {
    ...
    let trees = <View style={tw`px-4`}></View>
        ...
        for (let i = 0; i < entryGroupTrees.length; i++) {
            trees.push(
                <View key={entryGroupTrees[i].entry_group_id} className="tree-container">
                    <MainLeftMenuTreeNode {...entryGroupTrees[i]} pid="root" lvl={1} last={i + 1 === entryGroupTrees.length} parentLast={true}/>
                </View>
            )
        }
    }
        ...
        <View style={tw`bg-slate-900 text-slate-400 pr-1 h-full min-w-[20%]`}>
            ...
            <View style={tw`py-0 px-2 flex h-full mt-5 flex-1`}>
                <View style={
                    ...
                    , ...( entryGroupTreesStatus === ENTRY_GROUP_TREES_LOADED ? tw`hidden` : {})}
                }>
                    <View style={tw`flex flex-row justify-center`}>
                        <View style={tw`px-2`}>
                            <ActivityIndicator size="small" color="#e2e8f0" />
                        </View>
                        <Text>{(entryGroupTreesStatus === ENTRY_GROUP_TREES_UPDATING ? "updating" : "loading")}...</Text>
                    </View>
                </View>
