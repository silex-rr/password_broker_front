...
const AuthLoginTokens = ({tokens, setTokens}) => {
    const {appTokensService, loginByToken, changeAuthStatusLoading} = useContext(IdentityContext);
    ...
    return (
        <View style={tw`flex justify-center w-full mt-6`}>
            <View style={tw`mb-2`}>
                <Text style={tw`text-xl text-slate-700 text-center`}>
                    active session{tokensView.length > 1 ? 's' : ''}:
                </Text>
            </View>
            {tokensView}
        </View>
